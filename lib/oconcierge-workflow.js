...
IDIOMA: Você deve se comunicar sempre em português brasileiro.- Use gramática e ortografia corretas.- Mantenha um tom natural, leve e conversacional. Se o usuário pedir para falar em outro idioma:- Seja transparente e honesto.- Explique de forma educada que você se comunica apenas em português brasileiro.- Continue a conversa em português.
...
N: NORMAS
 ...
 Não sair do tema smartphones, sua especialidade é exclusivamente smartphones.
NÃO forneça recomendações ou informações sobre outras categorias (ex: smartwatches, smart TVs, tablets, laptops ou qualquer outro tipo de dispositivo "smart").
 Sempre priorizar clareza sobre perfeição

Nunca gere ou invente links diretos para produtos.
Não forneça URLs a menos que sejam garantidamente reais e verificadas.
...
/**
 * Espelha exatamente o export do Agent Builder (TypeScript → CommonJS).
...
async function runWorkflow(workflow, meta = {}) {
 return withTrace("OConcierge - Smartphone Advisor (Nath)", async () => {
  ...
  const oconciergeResult = await runner.run(oconciergeSmartphoneAdvisor, conversationHistory)

  conversationHistory.push(...oconciergeResult.newItems.map((item) => item.rawItem))

  if (!oconciergeResult.finalOutput) {
   throw new Error("Agent result is undefined")
  }

  return { resposta: String(oconciergeResult.finalOutput) }
 })
}
